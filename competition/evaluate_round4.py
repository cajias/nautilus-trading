"""
Round 4 Competition Evaluator
Hidden evaluation period: 2025-07-01 to 2025-09-30, $1,000 starting capital.
"""
...
AGENTS = [
    {
        ...
        "path": "/Users/rc/Projects/workspace/nautilus-trading/competition/agent-1-quant/round4/strategy.py",
        "module": "agent1_r4",
    },
    {
        "name": "Agent 2 - Sentiment (RSI Reversion + Volume Tournament)",
        "path": "/Users/rc/Projects/workspace/nautilus-trading/competition/agent-2-sentiment/round4/strategy.py",
        "module": "agent2_r4",
    },
    {
        "name": "Agent 3 - Macro (50-SMA + RSI Momentum)",
        "path": "/Users/rc/Projects/workspace/nautilus-trading/competition/agent-3-macro/round4/strategy.py",
        "module": "agent3_r4",
    },
    {
        "name": "Agent 4 - ML (115-Variant Tournament)",
        "path": "/Users/rc/Projects/workspace/nautilus-trading/competition/agent-4-ml/round4/strategy.py",
        "module": "agent4_r4",
    },
    {
        "name": "Agent 5 - Hybrid (Momentum Breakout Tournament)",
        "path": "/Users/rc/Projects/workspace/nautilus-trading/competition/agent-5-hybrid/round4/strategy.py",
        "module": "agent5_r4",
    },
]
...
def run_agent(agent: dict) -> dict:
    ...
    # Agent 5 needs special handling: run_tournament first, then run_backtest
    if agent["module"] == "agent5_r4":
        # Fetch data with buffer for indicators
        import pandas as pd
        buffer_start = (pd.Timestamp(EVAL_START) - pd.Timedelta(days=120)).strftime("%Y-%m-%d")
        df = mod.fetch_btc_daily(buffer_start, EVAL_END)
        print(f"Data: {df.index[0].date()} to {df.index[-1].date()}, {len(df)} bars")

        # Run tournament on a train period to select best strategy
        train_start = "2024-10-01"
        train_end = "2025-06-30"
        df_train = mod.fetch_btc_daily(
            (pd.Timestamp(train_start) - pd.Timedelta(days=120)).strftime("%Y-%m-%d"),
            train_end
        )
        print(f"Training data: {df_train.index[0].date()} to {df_train.index[-1].date()}, {len(df_train)} bars")
        best = mod.run_tournament(df_train, INITIAL_CAPITAL)
        if best is None:
            return {"error": "No viable strategy found in tournament"}
        strat_name, strat_cfg, _ = best
        print(f"Tournament winner: {strat_name} with config {strat_cfg}")

        # Now run the winning strategy on the eval period
    ...
    # Agent 4 returns train+test dict; pass eval dates as test period
    elif agent["module"] == "agent4_r4":
        raw = mod.run_backtest(start=EVAL_START, end=EVAL_END, initial_capital=INITIAL_CAPITAL)
        # Agent 4 returns {"train": {...}, "test": {...}}
        if "test" in raw:
            result = raw["test"]
        else:
            result = raw
...
def main():
    print("=" * 70)
    print("ROUND 4 COMPETITION EVALUATION")
    ...
    print("\n")
    print("=" * 70)
    print("ROUND 4 LEADERBOARD")
    print("=" * 70)
    print(f"{'Rank':<6}{'Agent':<50}{'Return %':>10}{'Equity':>10}{'Sharpe':>8}{'MaxDD%':>8}{'Trades':>8}{'WinR%':>8}")
    print("-" * 108)
    ...
    # Save results
    output_path = Path("/Users/rc/Projects/workspace/nautilus-trading/competition/round4_results.txt")
    with open(output_path, "w") as f:
        f.write("ROUND 4 COMPETITION RESULTS\n")
        f.write(f"Evaluation Period: {EVAL_START} to {EVAL_END}\n")
        f.write(f"Starting Capital: ${INITIAL_CAPITAL:,.0f}\n")
        f.write("=" * 70 + "\n\n")

        f.write(f"{'Rank':<6}{'Agent':<50}{'Return %':>10}{'Equity':>10}{'Sharpe':>8}{'MaxDD%':>8}{'Trades':>8}{'WinR%':>8}\n")
        f.write("-" * 108 + "\n")

        for i, r in enumerate(results, 1):
            err = " [ERROR]" if "error" in r else ""
            f.write(f"{i:<6}{r['agent'][:49]:<50}"
                    f"{r.get('total_return_pct', 0):>+10.2f}"
                    f"{r.get('final_equity', INITIAL_CAPITAL):>10.2f}"
                    f"{r.get('sharpe_ratio', 0):>8.2f}"
                    f"{r.get('max_drawdown_pct', 0):>8.2f}"

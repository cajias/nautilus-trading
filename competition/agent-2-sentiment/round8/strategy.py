"""
Agent 2 — Round 8: Conviction Concentration Sentiment Breakout

Thesis:
- R6 (+3.05) was my best result: aggressive momentum+panic across BTC/ETH/SOL.
- R7 went negative (-1.79). Diversification dilutes upside.
- Scoring: best single round wins. To beat +48% I need concentration.
- Strategy: tournament per asset on TRAIN, then put 100% capital on the
  single asset whose best config has the highest risk-adjusted return on TRAIN.
- Sentiment signals: taker-buy ratio surge, volume spikes, panic RSI reversal,
  OBV-confirmed breakouts. 4h bars.

TRAIN: 2024-07-01 to 2025-06-30
...
def tournament(df, symbol, capital):
    ...
    for cfg in param_grid():
        ...
        # Score: return scaled by drawdown penalty
...
def run_backtest(start: str, end: str, initial_capital: float = 1000.0) -> dict:
    """Train-tournament over assets, pick winner, concentrate TEST capital on it.

    For start/end covering TRAIN we return the tournament winner's TRAIN metrics.
    For a TEST call we require _selected passed via globals (see __main__).
    """
    buffer = (pd.Timestamp(start) - pd.Timedelta(days=20)).strftime("%Y-%m-%d")
    selected = globals().get("_LOCKED_CFG")

    if selected is None:
        # TRAIN: tournament per asset, pick best single asset to concentrate on
        results = {}
        for sym in SYMBOLS:
        ...
        # Pick the asset with the highest score (return - 0.3*dd)
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("TRAIN 2024-07-01 -> 2025-06-30")
    print("=" * 60)
    globals().pop("_LOCKED_CFG", None)
    train = run_backtest("2024-07-01", "2025-06-30", 1000.0)
    print("TRAIN:", {k: v for k, v in train.items()
                     if k not in ("trade_log", "config")})

    print("\n" + "=" * 60)
    print("TEST  2025-07-01 -> 2025-09-30")
    print("=" * 60)
    test = run_backtest("2025-07-01", "2025-09-30", 1000.0)
    ...
    output = {
        ...
        "round": 8,
        "approach": (
            "Conviction concentration: tournament momentum/panic sentiment "
            "breakout per asset on TRAIN, then lock capital 100% onto the "
            "single best-scoring asset (return - 0.3*dd) for TEST. "
            "Signals: EMA trend + volume surge + breakout + taker-buy ratio "
            "surge, plus oversold panic reversal."
        ),
        "train_period": ["2024-07-01", "2025-06-30"],
        "test_period": ["2025-07-01", "2025-09-30"],

...
Usage (smoke test against the template)::

    uv --project nautilus run python competition/evaluate_round11.py \\
        --round 11 --submission-dir competition/TEMPLATE

No network calls. If a ``ParquetDataCatalog`` path is supplied via
``--catalog-path``, bars come from there; otherwise a synthetic sine-wave
price series is generated so every submission has enough data to exercise
indicator warmup and signal firing.
"""
...
# ---------------------------------------------------------------------------
# Synthetic data (fallback when no ParquetDataCatalog is provided)
# ---------------------------------------------------------------------------


def _bar_type_interval_ns(bar_type: BarType) -> int:
    """Best-effort conversion of a BarType spec to nanoseconds per bar.

    Handles SECOND/MINUTE/HOUR/DAY/WEEK only (the aggregations we support
    on Binance Spot). Unknown aggregations default to 1 hour.
    """
    step = int(bar_type.spec.step)
    aggregation_name = str(bar_type.spec.aggregation).split(".")[-1].upper()
    per_step_ns = {
        "SECOND": 1_000_000_000,
        "MINUTE": 60 * 1_000_000_000,
        "HOUR": 3_600 * 1_000_000_000,
        "DAY": 24 * 3_600 * 1_000_000_000,
        "WEEK": 7 * 24 * 3_600 * 1_000_000_000,
    }.get(aggregation_name, 3_600 * 1_000_000_000)
    return step * per_step_ns
...
def generate_synthetic_bars(
    *,
    ...
    trend_pct_per_bar: float = 0.0015,
    amplitude_pct: float = 0.05,
    wavelength: int = 40,
) -> list[Bar]:
    """Return a sine-wave-on-trend synthetic bar series.

    Used when no ``ParquetDataCatalog`` is supplied. ``count`` must be large
    enough for any reasonable indicator warmup (>= 200 is comfortable for
    SMA30 / EMA50 strategies).
    """
    step_ns = _bar_type_interval_ns(bar_type)
    bars: list[Bar] = []
    for i in range(count):
        trend = base_price * (1.0 + trend_pct_per_bar * i)
        oscillation = amplitude_pct * trend * math.sin((2 * math.pi * i) / wavelength)
        ...
        bars.append(
            Bar(
                bar_type=bar_type,
                open=Price(mid, precision=price_precision),
                high=Price(high, precision=price_precision),
                low=Price(low, precision=price_precision),
                close=Price(mid, precision=price_precision),
                volume=Quantity(10.0, precision=size_precision),
                ts_event=ts,
                ts_init=ts,
            ),
        )
...
def load_catalog_bars(
    ...
    """Load bars + instrument from a ``ParquetDataCatalog``.

    Returns ``([], None)`` if either lookup fails so the caller can fall
    back to synthetic data. Never raises on missing data.
    """
    try:
        catalog = ParquetDataCatalog(str(catalog_path))
    except Exception as err:
        logger.warning("Could not open catalog at %s: %s", catalog_path, err)
        return [], None
    ...
    try:
        ...
        logger.warning("Catalog instrument lookup failed: %s", err)

    try:
        bars = catalog.bars(instrument_ids=[str(instrument_id)])
    except Exception as err:
        logger.warning("Catalog bar lookup failed: %s", err)
        bars = []

    # Filter to the requested bar_type. Some catalogs store multiple specs.
...
def _evaluate_pair(
    ...
    *,
    ...
    if ctx.catalog_path is not None:
        cat_bars, cat_instrument = load_catalog_bars(ctx.catalog_path, bar_type, instrument_id)
        if cat_bars:
            bars = cat_bars
            if cat_instrument is not None:
                # Prefer the catalog's own instrument so price_precision lines up.
                instrument = cat_instrument
        else:
            logger.warning(
                "Catalog at %s has no bars for %s; falling back to synthetic",
                ctx.catalog_path,
                bar_type,
            )

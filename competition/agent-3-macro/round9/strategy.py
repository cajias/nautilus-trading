"""
Agent 3 - Macro Strategist - Round 9
====================================
TRAIN: 2025-01-01..2025-12-31   TEST: 2026-01-01..2026-02-28
...
def fetch_klines(symbol: str, interval: str, start: str, end: str) -> list[dict]:
    ...
    while cur < end_ms:
        ...
        req = urllib.request.Request(f"{BINANCE_URL}?{qs}", headers={"User-Agent": "a3-macro-r9"})
...
def run_backtest(start: str, end: str, initial_capital: float = 1000.0) -> dict:
    ...
    for i, b in enumerate(btc):
        ...
        if units == 0:
            signal = None
            sym = None
            if dd > 0.22 and rs14_b[i] < 28:
                signal = "dip"
                sym = "BTC"
            else:
                # Prefer SOL if it's clearly outperforming and trend-up
                if trend_sol and ret20_s > max(ret20_b * 1.2, 0.03):
                    signal = "trend"
                    sym = "SOL"
                elif trend_btc:
                    signal = "trend"
...
def _fmt(label: str, r: dict, period: str) -> str:
    lines = [
        "=" * 60,
        f"AGENT 3 - MACRO STRATEGIST - ROUND 9 - {label}",
        f"Period: {period}",
        "-" * 60,
...
if __name__ == "__main__":
    train = run_backtest("2025-01-01", "2025-12-31")
    test = run_backtest("2026-01-01", "2026-02-28")
    out = (
        _fmt("TRAIN", train, "2025-01-01 to 2025-12-31")
        + "\n\n"
        + _fmt("TEST", test, "2026-01-01 to 2026-02-28")
    )
    print(out)
